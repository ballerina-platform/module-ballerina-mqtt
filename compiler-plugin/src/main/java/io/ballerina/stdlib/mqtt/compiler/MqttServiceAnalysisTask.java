 ...
 */
public class MqttServiceAnalysisTask implements AnalysisTask<SyntaxNodeAnalysisContext> {
    private final MqttServiceValidator serviceValidator;

    public MqttServiceAnalysisTask() {
        this.serviceValidator = new MqttServiceValidator();
    }
    ...
    private boolean isMqttService(SyntaxNodeAnalysisContext context) {
        ...
        if (symbol.isPresent()) {
            ...
            if (listeners.size() > 1 && hasMqttListener(listeners)) {
                context.reportDiagnostic(PluginUtils.getDiagnostic(
                        PluginConstants.CompilationErrors.INVALID_MULTIPLE_LISTENERS, DiagnosticSeverity.ERROR,
                ...
                if (listeners.get(0).typeKind() == TypeDescKind.UNION) {
                    ...
                    for (TypeSymbol memberSymbol : members) {
                        ...
                        if (module.isPresent()) {
                            isMqttService = validateModuleId(module.get());
                        }
                    }
                } else {
